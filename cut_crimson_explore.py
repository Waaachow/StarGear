# Transcribe -> align -> cut audio/Explore/Crimson_Explore.mp3 (CHATTER.crimsonIdle, 5 lines)
# into audio/Explore/crimson_idle_NN.mp3. Same route as align_explore.py/cut_explore.py, but
# standalone since Crimson isn't in that pipeline's fixed name list.
#
#   python cut_crimson_explore.py
import os, re, json, difflib, subprocess
from faster_whisper import WhisperModel

HERE = os.path.dirname(os.path.abspath(__file__))
SRCDIR = os.path.join(HERE, "audio", "Explore")
...
LINES = [
    "Marvelous view. Almost as good as my entrance music.",
    "Relax, Captain — the Comet doesn't do 'boring.'",
    "You know, back home they'd be selling tickets to watch this.",
    "Keep flying like that and I'll have to update my highlight reel.",
    "Quiet sector. Good — saves the fireworks for when it counts.",
]

def toks(s):
    return [t for t in re.sub(r"[^a-z0-9 ]", " ", s.lower().replace("'", "")).split() if t]

src = os.path.join(SRCDIR, "Crimson_Explore.mp3")
...
for i, sp in enumerate(spans):
    ...
    out = os.path.join(SRCDIR, f"crimson_idle_{num:02d}.mp3")
    subprocess.run([FFM, "-y", "-loglevel", "error", "-ss", f"{a:.3f}", "-to", f"{b:.3f}",
                    ...
                    out], check=True)
    print(f"   {num:2d}. {a:6.2f} -> {b:6.2f}  {LINES[i]}  -> {os.path.basename(out)}")
