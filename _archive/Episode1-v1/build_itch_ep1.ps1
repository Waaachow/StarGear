# Builds the itch.io HTML bundle for Star Gear - Episode 1 - Echoes of Steel.
# Produces "<title> - v0.1.zip" in the project root with index.html at the zip root.
# Forked from build_itch.ps1 (Episode 0) — same staging/compression pipeline, pointed
# at ep1.html/ep1_flight.html instead of episode1.html/iso_grid_prototype.html.
...
$build = Join-Path $root "dist\build_ep1"
$A     = Join-Path $build "Assets"
$index = Join-Path $build "index.html"
$zip   = Join-Path $root "StarGear - Episode 1 - Echoes of Steel - v0.1.zip"

# ---------- 1. stage a clean tree ----------
if (Test-Path $build) { Remove-Item $build -Recurse -Force }
New-Item -ItemType Directory -Path $build -Force | Out-Null
Copy-Item (Join-Path $root "ep1.html")        (Join-Path $build "index.html")
Copy-Item (Join-Path $root "ep1_flight.html") $build
# shared Assets/audio trees — same folders Episode 0 ships, since Episode 1 reuses
# returning-crew art, music and SFX from them (see the asset-storage plan)
...
# drop dev leftovers from the staged copy (same patterns as the Episode 0 build)
...
$pretrim = Join-Path $build "audio\Episode0\vo\_pretrim_backup"
if (Test-Path $pretrim) { Remove-Item $pretrim -Recurse -Force }
Get-ChildItem (Join-Path $build "audio") -Recurse -File | Where-Object {
  $_.Name -match '(_old|_orig)\.mp3$' -or $_.Name -in @("Kael_Combat_v1.mp3", "Kael_Combat_v2.mp3")
} | ForEach-Object { [System.IO.File]::Delete($_.FullName) }

# ---------- 2. image compression helpers (identical to build_itch.ps1) ----------
...
# ---------- 3. compress ----------
$converted = @()
# CGs: same 1920w/q88 cap as Episode 0. Covers whichever of CG01–CG08 have landed —
# CG01 (Burning Convoy) is delivered as of this build; the rest are still placeholders
# under Assets/cg/ (convention path) and simply won't be present here yet.
foreach ($f in Get-ChildItem "$A\CG" -File -Filter *.png) {
  $converted += (To-Jpeg $f.FullName 1920 88)
}
# Named backgrounds actually referenced by ep1.html — Bridge/Bridge Red Alert are
# shared with Episode 0; THANKS/DEFEAT are the end-screen backdrops. (Episode 0's
# Bg03/bg04/bg06/START_SCREEN aren't referenced by Episode 1 — see BG_OVERRIDE in
# ep1.html — so they're intentionally left off this list.)
foreach ($n in @("Bridge.png","Bridge Red Alert.png","THANKS.png","DEFEAT.png")) {
...
# char/* left at native PNG (shown large, need alpha) — same as Episode 0

# rewrite .png -> .jpg refs (plain replace so em/en-dashes match verbatim).
# NOTE: the four Episode 0 station dock interiors (Waypoint_Hub/Veil_Anchorage/
# Quarry_Station/Salvage_Reach.png) are loaded via a DYNAMIC path in ep1_flight.html
# (inherited unchanged from iso_grid_prototype.html), so the literal replace below
# can't reach them — they stay PNG, same gotcha as the Episode 0 build.
foreach ($doc in @($index, (Join-Path $build "ep1_flight.html"))) {
